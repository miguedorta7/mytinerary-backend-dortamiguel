...
async function createCities(arrayCities){
    try {
        ...
        for (let city of arrayCities){
            let user  =await User.findOne({ mail:city.admin_id }) //Busco el usuario que coincida el mail del objeto, solo necesito el ID de ese usuario
            let admin_id = await user._id                         //Reasigno el valor del admin_id de cada ciudad para cambiar el mail que viene por default POR LO QUE CORRESPONDE (que seria el id)      
